function [aif, regionpixels] = DSC_mri_aif(conc, mask, options)

options.display = 0;


% Identifies the AIF for DSC-MRI examinations. The method is designed to identify the
% middle cerebral artery (MCA) in slices immediately superior to the corpus callosum.
%
% Input parameters:
% - conc (4D matrix) containing the concentration time course of all
% voxels
% - mask (3D matrix) containing the mask for each slice. NB: the mask
% is not the one used for concentration calculation, but a restrictive version of it
% (the fill function was not used)
% - options is the struct containing method options, the significant ones are:
%
% options.aif.enable: flag enabling AIF search (default=1)
%
% options.aif.semiasseMaggiore: allows identifying the AIF search area. The elliptical search area
% has a major axis equal to two times the portion indicated by this
% option of the brain size (default=0.35)
%
% options.aif.semiasseMinore: similar to the major axis, but relative
% to the other axis of the search region
% (default=0.15)
%
...
% (default=4)
%
% Output parameters: aif structure, containing
% - ROI
...
[aif_old,regionpixels] = estraiAIF(reshape(conc(:,:,options.aif.nSlice,:),options.nR,options.nC,options.nT),mask(:,:,options.aif.nSlice),options);
aif=aif_old;
...
%%
function [AIF,regionpixels]=estraiAIF(AIFslice,mask,options)
